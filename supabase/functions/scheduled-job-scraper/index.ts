      ...
      try {
        ...
        for (let index = 0; index < indeedJobs.length; index++) {
          ...
          if (!extractedJobTitle) {
            console.log(`Skipping job ${index} for ${jobTitle}: No valid positionName found or failed validation. Raw title: "${job.PositionName || job.positionName || job.jobTitle || 'N/A'}"`);
            skippedCount++;
            continue;
          }
          
          console.log(`Accepted job ${index} for ${jobTitle}: "${extractedJobTitle}" from positionName field`);
