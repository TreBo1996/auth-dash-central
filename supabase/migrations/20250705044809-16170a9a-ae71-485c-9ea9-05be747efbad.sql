-- Enhanced search_jobs function with better title prioritization and relevance scoring
...
BEGIN
    ...
    CASE 
      ...
      ELSE 
        ...
        (
          ...
          CASE WHEN to_tsvector('english', j.title) @@ websearch_to_tsquery('english', normalized_query) THEN 5.0 ELSE 0.0 END +
          -- Base weight for full-text search across all fields (1x)
          ts_rank(j.search_vector, websearch_to_tsquery('english', normalized_query)) +
  ...
  WHERE 
    ...
    AND (
      ...
      j.search_vector @@ websearch_to_tsquery('english', normalized_query)
    )
