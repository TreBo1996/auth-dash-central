-- Create the missing fast_search_jobs function and hot_job_searches materialized view

-- First, refresh the materialized view for hot searches (updated every 15 minutes)
...
CREATE INDEX IF NOT EXISTS idx_hot_job_searches_title ON public.hot_job_searches (title);
CREATE INDEX IF NOT EXISTS idx_hot_job_searches_company ON public.hot_job_searches (company);
CREATE INDEX IF NOT EXISTS idx_hot_job_searches_location ON public.hot_job_searches (location);
CREATE INDEX IF NOT EXISTS idx_hot_job_searches_relevance ON public.hot_job_searches (pre_calculated_relevance DESC);

-- Create the fast_search_jobs function
