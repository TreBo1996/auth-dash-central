...
import { FileText, Mail, CheckCircle, Award, TrendingUp, Target } from 'lucide-react';
...
export const JobHubMetrics: React.FC<JobHubMetricsProps> = ({
  jobs
}) => {
  ...
  const jobsWithCompleteStack = jobs.filter(job => job.optimized_resumes && job.optimized_resumes.length > 0 && job.cover_letters && job.cover_letters.length > 0).length;

  // Calculate average ATS score
  const allResumesWithScores = jobs.flatMap(job => job.optimized_resumes?.filter(resume => resume.ats_score) || []);
  const averageAtsScore = allResumesWithScores.length > 0 ? Math.round(allResumesWithScores.reduce((sum, resume) => sum + resume.ats_score, 0) / allResumesWithScores.length) : 0;
  const metrics = [{
  ...
  return <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
      {metrics.map((metric, index) => <Card key={index} className="hover:shadow-md transition-shadow">
          ...
          <CardContent>
            ...
            <p className="text-gray-500 mx-0 text-xs font-bold">
              {metric.description}
            </p>
          </CardContent>
        </Card>)}
