...
export const ColorSchemeSelector: React.FC<ColorSchemeSelectorProps> = ({
  ...
  return <div className="space-y-3">
      ...
      <div className="space-y-2">
        {colorSchemes.map(scheme => <Card key={scheme.id} className={`cursor-pointer transition-all hover:shadow-sm ${selectedScheme === scheme.id ? 'ring-2 ring-blue-500' : ''}`} onClick={() => onSchemeSelect(scheme.id)}>
            <CardContent className="p-3">
              <div className="flex items-center justify-between">
                ...
                {selectedScheme === scheme.id && <div className="w-2 h-2 bg-blue-500 rounded-full" />}
              </div>
            </CardContent>
          </Card>)}
      </div>
